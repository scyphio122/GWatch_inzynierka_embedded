...
#define BLOCK_COUNT_CHECK(COUNT, SIZE)                                                            \
        if (((COUNT) == 0) ||                                                                     \
            ...
            ((((COUNT) * (SIZE)) > PSTORAGE_FLASH_PAGE_SIZE_BYTES) && (PSTORAGE_FLASH_PAGE_SIZE_BYTES % (SIZE))))  \
        {                                                                                         \
            return NRF_ERROR_INVALID_PARAM;                                                       \
        }
    ...
    switch (m_swap_state)
    {
        case STATE_DATA_TO_SWAP_WRITE:
            ...
            retval = sd_flash_write((uint32_t *)(PSTORAGE_SWAP_ADDR),
                                    (uint32_t *)(page_number * PSTORAGE_FLASH_PAGE_SIZE_BYTES),
                                    PSTORAGE_FLASH_PAGE_SIZE_BYTES / sizeof(uint32_t));
            if (retval == NRF_SUCCESS)
            {
                m_swap_state = STATE_DATA_ERASE;
            }
        ...
        case STATE_HEAD_RESTORE:
            ...
            retval = sd_flash_write((uint32_t *)(page_number * PSTORAGE_FLASH_PAGE_SIZE_BYTES),
        ...
        case STATE_SWAP_ERASE:
            ...
            retval = sd_flash_page_erase(PSTORAGE_SWAP_ADDR / PSTORAGE_FLASH_PAGE_SIZE_BYTES);
            if (retval == NRF_SUCCESS)
            {
                m_swap_state = STATE_COMPLETE;
            }
...
static uint32_t cmd_process()
{
    ...
    switch (p_cmd->op_code)
    {
        ...
        case PSTORAGE_CLEAR_OP_CODE:
        {
            ...
            {
                page_number = ((storage_addr / PSTORAGE_FLASH_PAGE_SIZE_BYTES) + m_round_val);
            ...
            {
                page_number = (storage_addr / PSTORAGE_FLASH_PAGE_SIZE_BYTES);

                uint32_t head_word_size = (
                    storage_addr -
                    (page_number * PSTORAGE_FLASH_PAGE_SIZE_BYTES)
                    ) / sizeof(uint32_t);

                uint32_t tail_word_size = (
                    ((page_number + 1) * PSTORAGE_FLASH_PAGE_SIZE_BYTES) -
        ...
        {
            uint32_t page_number = (storage_addr / PSTORAGE_FLASH_PAGE_SIZE_BYTES);
                           ...
                           pstorage_handle_t       * p_block_id)
{
    ...
    {
        page_count++;
        if (total_size > PSTORAGE_FLASH_PAGE_SIZE_BYTES)
        {
            total_size -= PSTORAGE_FLASH_PAGE_SIZE_BYTES;
        }
        else
        {
            total_size = 0;
        }
        m_next_page_addr += PSTORAGE_FLASH_PAGE_SIZE_BYTES;
    }
    while (total_size > 0);
